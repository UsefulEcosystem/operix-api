...
const saleProjection = 'id, customer_name, customer_document, customer_phone, total_amount, notes, sold_at';
const itemProjection = 'id, stock_id, item_name, item_code, serial_number, quantity, unit_price, total_price';
  ...
  static async listar(tenantId: number) {
    ...
    try {
      const result = await connect.query(
        `SELECT s.id, s.customer_name, s.customer_document, s.customer_phone, s.total_amount, s.notes, s.sold_at,
                COALESCE(
  ...
  static async criar(tenantId: number, data: SaleCreateDto) {
    ...
    try {
      ...
      const saleResult = await connect.query(
        ...
        [tenantId, data.customer_name, data.customer_document || null, data.customer_phone || null, data.notes || null, soldAt],
      );
      const sale = saleResult.rows[0];
      const items = [];
      let totalAmount = 0;
      ...
      for (const item of data.items) {
        ...
        const itemResult = await connect.query(
          `INSERT INTO sale_items
           (tenant_id, sale_id, stock_id, item_name, item_code, serial_number, quantity, unit_price, total_price, created_at)
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, NOW())
           RETURNING ${itemProjection}`,
          [tenantId, sale.id, stock.id, stock.name, stock.code, item.serial_number || null, item.quantity, unitPrice, totalPrice],
        );
